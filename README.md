...
```txt
homelab-ansible
[cite_start]├── inventory.ini           # 定义物理机与虚拟机的 IP 及连接方式 [cite: 1]
├── group_vars/             # 分组变量配置（包含加密的认证信息）
├── roles/
[cite_start]│   ├── pve_host            # PVE 宿主机：网络转发、校园网认证 [cite: 24]
[cite_start]│   ├── vm_init             # VM 初始化：环境清理、磁盘挂载 [cite: 35]
[cite_start]│   └── deploy_services     # Docker 应用：全家桶部署及监控发现 [cite: 9]
└── site.yml                # 总调度入口文件
...
2. PVE 宿主机已开启 SSH 访问。
3. [cite_start](https://www.google.com/search?q=%E6%8E%A8%E8%8D%90) 已建立 Tailscale 连接以实现安全远程管理 。
